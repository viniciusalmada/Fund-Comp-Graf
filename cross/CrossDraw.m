...
classdef CrossDraw < handle
    ...
    methods (Access = protected) % Access from methods in subclasses
        ...
        function drawSupport(draw,canvas,x,y,color,fixRotation,vargin)
            if (fixRotation==1)
    ...
    methods
        ...
        function deformedConfig(draw,cnv)
            ...
            for i=1:draw.solver.nmemb
                ...
                if i==1 %% First member
                    ...
                    if (draw.solver.supinit == 0)
                        v01 = q/EI * ((-L3/48).*x + (L/16).*x.^3 - (1/24).*x.^4);
                        N2 = zeros(1,totalDisc);
                        N4 = (-1/2).*x + (1/(2*L2)).*x.^3;
                        initNodeRot = (q/EI * (-L3/48)) + (-1/2 + 1/(2*L2)*L3) * endNodeRot;
                    else
                        v01 = q/EI * ((-L2/24).*x.^2 + (L/12).*x.^3 - (1/24).*x.^4);
                        N2 = x - (2/L) .* x.^2 + (1/L2) .* x.^3;
                        N4 = (-1/L).*x.^2 + (1/L2).*x.^3;
                        initNodeRot = 0;
                    end
                elseif i==draw.solver.nmemb %% Last member
                    ...
                    if (draw.solver.supend == 0)
                        v01 = q/EI * ((-L2/16).*x.^2 + (5*L/48).*x.^3 - (1/24).*x.^4);
                        N2 = x - (3/(2*L)).*x.^2 + (1/(2*L2)).*x.^3;
                        N4 = zeros(1,totalDisc);
                        endNodeRot = (q/EI * ((-L3/8) + (5*L3/16) - (L3/6))) + ...
                            (3*L^2/(2*L^2) - 2) * initNodeRot;
                    else
                        v01 = q/EI * ((-L2/24).*x.^2 + (L/12).*x.^3 - (1/24).*x.^4);
                        N2 = x - (2/L) .* x.^2 + (1/L2) .* x.^3;
                        N4 = (-1/L).*x.^2 + (1/L2).*x.^3;
            ...
            line(cnv,x0,v0.*factor,'Color',[1 0 1]);
            %%%%%%% COMPLETE HERE - CROSSDRAW: 02 %%%%%%%
        end
